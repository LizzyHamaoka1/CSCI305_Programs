...
fun listCities() : List<City> {
    return listOf(City("Atlanta", 33.7490, -84.3880), 
...
fun distanceFromSeattle(city: City) : Double {
    return haversine( 47.6062, -122.3321, city.longitude, city.latitude)
}
...
fun main() {
    ...
    println("\n" + "*********Step 3***********")
    println(
        "The distance between Seattle and Tacoma is " + distanceFromSeattle(
            City(
                "Tacoma",
                47.2529,
                -122.4443
    ...
    println("\n" + "*********Step 5***********")
    println("List of cities over 50km from Seattle")
    var distance = cityLocations.filter { city -> distanceFromSeattle(city) > 50 }
    ...
    var c = "Bozeman"
    if(mappedCities.containsKey(c)) {
        println("\n" + "The latitude for $c is " + mappedCities.getValue(c).latitude)
    }
}
